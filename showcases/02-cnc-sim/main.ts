...
/** Indiziertes Dreiecksgitter (N×N Punkte) für das Höhenfeld. */
...
/** AABB des Werkzeug-Fußabdrucks in Pixelkoordinaten (optimierter Dispatch). */
...
/** Schreibt ToolUniforms (32 Byte) an Byte-Offset `off`. */
...
// ----- Werkzeug-Rotationskörper (Overlay) -----
...
function generateToolMesh(move: ToolMove): Float32Array {
  const cx = move.x;
  const cz = move.y;
  const r = move.toolRadius;
  const tipY = move.cutZ * HEIGHT_SCALE;
  ...
  for (let i = 0; i < S; i++) {
    ...
    tri(p0b, n0, p1b, n1, p1t, n1);
    tri(p0b, n0, p1t, n1, p0t, n0);
  }
  // Deckel
...
async function main(): Promise<void> {
  ...
  // ----- Bind-Group-Layouts -----
  const carveBGL = device.createBindGroupLayout({
    label: "carve-bgl",
    entries: [
  ...
  const toolBGL = device.createBindGroupLayout({
    label: "tool-bgl",
    entries: [
  ...
  function rebuildForResolution(size: number): void {
    ...
    carveBindGroup = device.createBindGroup({
      label: "carve-bg", layout: carveBGL,
      entries: [
    ...
    fieldBindGroup = device.createBindGroup({
      label: "field-bg", layout: fieldBGL,
      entries: [
        { binding: 0, resource: { buffer: zmapBuffer } },
  ...
  /** Setzt die Z-Map auf Rohteilhöhe (1.0) und die Wiedergabe zurück. */
  ...
  /** Wendet die Schritte [from, to) auf die Z-Map an (ein Pass pro Schritt). */
  ...
  canvas.addEventListener("touchmove", (e) => {
    ...
    cam.elevation = Math.max(0.05, Math.min(Math.PI / 2 - 0.05, cam.elevation - (e.touches[0].clientY - lastMouse.y) * 0.006));
    lastMouse = { x: e.touches[0].clientX, y: e.touches[0].clientY };
  }, { passive: false });
  ...
  const params = {
    ...
    reset(): void {
      ...
      params.playing = true;
      playingCtrl.updateDisplay();
    },
  };
  const gui = new GUI({ title: "CNC Simulation" });
  const playingCtrl = gui.add(params, "playing").name("Abspielen");
  gui.add(params, "speed", 1, 200, 1).name("Schritte / Frame");
  const progressCtrl = gui
    .add(params, "progress", 0, toolpath.length, 1)
    .name("Fortschritt")
  ...
  function updateCamera(): void {
    ...
    mat4.lookAt(view, [ex, ey, ez], [0, 0.30, 0], [0, 1, 0]);
  ...
  // ----- Render-Loop (rAF, sonst setTimeout-Fallback) -----
