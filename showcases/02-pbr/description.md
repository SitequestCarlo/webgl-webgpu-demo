...
Die **Bidirectional Reflectance Distribution Function (BRDF)** beschreibt, wie viel Licht von einer Oberfläche in Richtung des Betrachters reflektiert wird:

$$f_r = \frac{D \cdot G \cdot F}{4 \cdot (N \cdot V) \cdot (N \cdot L)}$$

Die ausgehende Strahldichte ergibt sich durch Einsetzen in die Reflektanzgleichung:

$$L_o = f_r \cdot L_i \cdot (N \cdot L) = \frac{D \cdot G \cdot F}{4 \cdot (N \cdot V) \cdot (N \cdot L)} \cdot L_i \cdot (N \cdot L)$$
