// Path-Tracer-Shader für Showcase 04 (WebGL2).
// Szene: Cornell Box — 5 Wände, 1 Flächenlicht, 2 Boxen.
// Algorithmus: Monte-Carlo-Path-Tracing mit kosinus-gewichteter Halbkugel-Stichprobe.
// Ping-Pong-Akkumulation über uAccum-Textur.
...
// ---------------------------------------------------------------------------
export const PT_FS = /* glsl */`#version 300 es
precision highp float;
precision highp int;
...
// ---- Offene Szene (4 Wände + Boden, kein AABB, kein Dach) ---------------
struct Hit { float t; vec3 n; vec3 albedo; };

bool hitYPlane(vec3 ro, vec3 rd, float y, float x0, float x1, float z0, float z1, float tmax, out float t) {
...
bool hitXPlane(vec3 ro, vec3 rd, float x, float y0, float y1, float z0, float z1, float tmax, out float t) {
  if (abs(rd.x) < 1e-6) return false;
  t = (x - ro.x) / rd.x;
  if (t < 0.001 || t > tmax) return false;
  vec3 p = ro + t * rd;
  return p.y >= y0 && p.y <= y1 && p.z >= z0 && p.z <= z1;
}
bool hitZPlane(vec3 ro, vec3 rd, float z, float x0, float x1, float y0, float y1, float tmax, out float t) {
...
// AABB ohne out-Parameter (D3D11/ANGLE-kompatibel)
...
vec3 sky(vec3 rd) {
  float s = clamp(rd.y * 0.5 + 0.5, 0.0, 1.0);
  return mix(vec3(0.85, 0.82, 0.75), vec3(0.45, 0.68, 1.0), s * s);
}

// 2 Bounces — minimal für ANGLE/D3D11-Kompatibilität
vec3 pathTrace(vec3 ro, vec3 rd) {
  Hit h0 = scene(ro, rd);
  if (h0.t >= INF) return sky(rd);
  vec3 throughput = h0.albedo;
  vec3 ro1 = ro + h0.t * rd + h0.n * 0.001;
  vec3 rd1 = cosineSample(h0.n);
  Hit h1 = scene(ro1, rd1);
  if (h1.t >= INF) return throughput * sky(rd1);
  vec3 ro2 = ro1 + h1.t * rd1 + h1.n * 0.001;
  vec3 rd2 = cosineSample(h1.n);
  Hit h2 = scene(ro2, rd2);
  if (h2.t >= INF) return throughput * h1.albedo * sky(rd2);
  return vec3(0.0);
}
...
void main() {
  ...
  // Running Average via Alpha-Blending: src_alpha = 1/(N+1)
  // gl.blendFunc(SRC_ALPHA, ONE_MINUS_SRC_ALPHA) akkumuliert korrekt.
  vec3 mapped = newSample / (newSample + vec3(1.0));
...
// ---------------------------------------------------------------------------
// Blit-Shader: liest Running-Average direkt aus der Textur
// ---------------------------------------------------------------------------
...
void main() {
  ...
  vec4 acc = texture(uAccum, uv);
  // Debug-Overlay: zeige Grün wenn Akkumulation > 0, sonst lese Akkumulation
  if (acc.a > 0.001) {
    fragColor = acc;
  } else {
    // Noch keine Akkumulation: UV als Gradient anzeigen
