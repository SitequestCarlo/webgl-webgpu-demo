...
// Die aktive Anzahl wird per uNumLights-Uniform gesetzt � kein Shader-Rebuild n�tig.
// Pro Frame entstehen N � 2 gl.uniform3f()-Aufrufe: API-Overhead w�chst linear mit N.
 ...
 * WebGL2-Limit: 2�MAX_LIGHTS vec3-Uniforms m�ssen in MAX_FRAGMENT_UNIFORM_VECTORS passen.
 * Bei 1024 vec4-Slots (Minimum-Garantie) und 2 Arrays � N vec4 bleiben ~500 nutzbar.
...
gl.enable(gl.CULL_FACE);           // Backface Culling: R�ckseiten nicht zeichnen
...
// w�re zu teuer (N zus�tzliche String-Lookups pro Frame).
...
/** Erzeugt Startfarben und Positionen f�r n Lichtquellen auf einer Kreisbahn. */
...
async function render(now: number): Promise<void> {
  ...
  // Jeder gl.uniform*-Aufruf ist ein JS?Native-�bergang � der API-Overhead w�chst
  // linear mit N (Gegenst�ck zu WebGPUs einzelnem writeBuffer).
  cpuTimer.begin();
  ...
  // Jeder Aufruf ist ein JS?Native-�bergang � das ist der Kernunterschied zu WebGPU.
  ...
  stats.update();
  if (benchmark.isRunning) await glFenceAsync(gl); // GPU-Sync (async) ? Timer-Query verf�gbar
