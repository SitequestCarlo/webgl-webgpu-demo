...
// Automatischer Sweep über alle N-Werte → CSV-Download
let sweepBenchmark: BenchmarkRun | null = null;
const N_SWEEP = [1, 2, 4, 8, 16, 32, 64, 128, 256];
gui.add({ sweep: async () => {
  resultsEl.style.display = "block";
  const rows = ["Lichter;avg_ms;avg_fps;p95_ms;min_ms;max_ms"];
  sweepBenchmark = new BenchmarkRun(15, 60);
  for (const n of N_SWEEP) {
    params.numLights = n;
    lights = buildLights(n);
    let sumMs = 0, sumP95 = 0, sumMin = 0, sumMax = 0;
    for (let run = 0; run < 5; run++) {
      resultsEl.textContent = `Sweep: N=${n} Lichtquellen ... (Lauf ${run + 1}/5)`;
  ...
  a.download = "lights-webgl.csv"; a.click(); URL.revokeObjectURL(a.href);
...
function render(now: number): void {
  ...
  sweepBenchmark?.sample(now);  // nur aktiv während Auto-Sweep
