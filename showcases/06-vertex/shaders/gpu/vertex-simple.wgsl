// vertex-simple.wgsl – Standard Blinn-Phong (WebGPU, Showcase 06)
// GPU-Timestamps via timestamp-query messen echte GPU-Zeit (ns-genau).
struct Scene {
  view:       mat4x4<f32>,
  proj:       mat4x4<f32>,
  lightPos:   vec4<f32>,
...
struct Draw { model: mat4x4<f32>, normalMat: mat4x4<f32>, color: vec4<f32>, _pad: array<vec4<f32>,7> }
@group(0)@binding(0)var<uniform>scene:Scene; @group(1)@binding(0)var<uniform>draw:Draw;
...
@vertex fn vs_main(@location(0)pos:vec3<f32>,@location(1)norm:vec3<f32>)->VsOut{
    let w=draw.model*vec4<f32>(pos,1.0);var o:VsOut;
...
@fragment fn fs_main(in:VsOut)->@location(0)vec4<f32>{
    ...
    let col=scene.lightColor.w*draw.color.xyz+diff*draw.color.xyz*scene.lightColor.xyz+spec*scene.lightColor.xyz;
