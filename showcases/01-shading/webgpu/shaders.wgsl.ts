// WGSL-Shader für Showcase 01 – alle 7 Shading-Modi.
// Bind-Group-Layout (identisch für alle Modi):
//   @group(0) @binding(0) : Transform-Uniform-Buffer (model, view, proj, normalMat als mat4)
//   @group(0) @binding(1) : Material-Uniform-Buffer  (color/ambient, lightPos/shininess,
//                                                     viewPos/toonSteps, lightColor/roughness, metallic)

export type ShadingMode = "none" | "flat" | "gouraud" | "phong" | "blinn-phong" | "toon" | "pbr";
export const SHADING_MODES: ShadingMode[] = ["none", "flat", "gouraud", "phong", "blinn-phong", "toon", "pbr"];

// --- Gemeinsame WGSL-Blöcke -------------------------------------------

const BINDINGS = /* wgsl */`
struct Transform {
    model:     mat4x4<f32>,
    view:      mat4x4<f32>,
    proj:      mat4x4<f32>,
    normalMat: mat4x4<f32>,
}
struct Material {
    colorAmbient:    vec4<f32>,  // xyz = Farbe,      w = ambient
    lightPosShiny:   vec4<f32>,  // xyz = lightPos,   w = shininess
    viewPosToon:     vec4<f32>,  // xyz = viewPos,    w = toonSteps
    lightColorRough: vec4<f32>,  // xyz = lightColor, w = roughness
    metallic:        f32,
    _p0: f32, _p1: f32, _p2: f32,
}
@group(0) @binding(0) var<uniform> tfm: Transform;
@group(0) @binding(1) var<uniform> mtl: Material;
`;

// Vertex-Shader: gibt worldPos + Normale weiter (none, phong, blinn-phong, toon, pbr).
const VS_PASS = /* wgsl */`
struct VsOut {
    @builtin(position) clip:     vec4<f32>,
    @location(0)       worldPos: vec3<f32>,
    @location(1)       normal:   vec3<f32>,
}
@vertex fn vs_main(
    @location(0) pos:  vec3<f32>,
    @location(1) norm: vec3<f32>,
) -> VsOut {
    let world = tfm.model * vec4<f32>(pos, 1.0);
    var o: VsOut;
    o.clip     = tfm.proj * tfm.view * world;
    o.worldPos = world.xyz;
    o.normal   = (tfm.normalMat * vec4<f32>(norm, 0.0)).xyz;
    return o;
}
`;

// Blinn-Phong Beleuchtungsfunktion (Half-Vector).
const FN_BLINNPHONG = /* wgsl */`
fn blinnPhong(N: vec3<f32>, worldPos: vec3<f32>) -> vec3<f32> {
    let L = normalize(mtl.lightPosShiny.xyz - worldPos);
    let V = normalize(mtl.viewPosToon.xyz  - worldPos);
    let H = normalize(L + V);
    let diff = max(dot(N, L), 0.0);
    let spec = pow(max(dot(N, H), 0.0), mtl.lightPosShiny.w);
    return mtl.colorAmbient.w * mtl.colorAmbient.xyz
         + diff * mtl.colorAmbient.xyz * mtl.lightColorRough.xyz
         + spec * mtl.lightColorRough.xyz;
}
`;

// Klassisches Phong (Reflect-Vektor).
const FN_PHONG = /* wgsl */`
fn shadingPhong(N: vec3<f32>, worldPos: vec3<f32>) -> vec3<f32> {
    let L = normalize(mtl.lightPosShiny.xyz - worldPos);
    let V = normalize(mtl.viewPosToon.xyz  - worldPos);
    let R = reflect(-L, N);
    let diff = max(dot(N, L), 0.0);
    let spec = pow(max(dot(R, V), 0.0), mtl.lightPosShiny.w);
    return mtl.colorAmbient.w * mtl.colorAmbient.xyz
         + diff * mtl.colorAmbient.xyz * mtl.lightColorRough.xyz
         + spec * mtl.lightColorRough.xyz;
}
`;

// --- Shader-Module pro Modus ------------------------------------------

const SHADER_NONE = BINDINGS + VS_PASS + /* wgsl */`
@fragment fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    return vec4<f32>(mtl.colorAmbient.xyz, 1.0);
}`;

// Flat: Beleuchtung pro Vertex, @interpolate(flat) -> eine Farbe pro Dreieck.
const SHADER_FLAT = BINDINGS + FN_BLINNPHONG + /* wgsl */`
struct VsOutFlat {
    @builtin(position)                        clip:  vec4<f32>,
    @location(0) @interpolate(flat, first)    color: vec3<f32>,
}
@vertex fn vs_main(@location(0) pos: vec3<f32>, @location(1) norm: vec3<f32>) -> VsOutFlat {
    let world = tfm.model * vec4<f32>(pos, 1.0);
    let N = normalize((tfm.normalMat * vec4<f32>(norm, 0.0)).xyz);
    var o: VsOutFlat;
    o.clip  = tfm.proj * tfm.view * world;
    o.color = blinnPhong(N, world.xyz);
    return o;
}
@fragment fn fs_main(in: VsOutFlat) -> @location(0) vec4<f32> {
    return vec4<f32>(in.color, 1.0);
}`;

// Gouraud: Beleuchtung pro Vertex, glatt interpoliert.
const SHADER_GOURAUD = BINDINGS + FN_BLINNPHONG + /* wgsl */`
struct VsOutSmooth {
    @builtin(position) clip:  vec4<f32>,
    @location(0)       color: vec3<f32>,
}
@vertex fn vs_main(@location(0) pos: vec3<f32>, @location(1) norm: vec3<f32>) -> VsOutSmooth {
    let world = tfm.model * vec4<f32>(pos, 1.0);
    let N = normalize((tfm.normalMat * vec4<f32>(norm, 0.0)).xyz);
    var o: VsOutSmooth;
    o.clip  = tfm.proj * tfm.view * world;
    o.color = blinnPhong(N, world.xyz);
    return o;
}
@fragment fn fs_main(in: VsOutSmooth) -> @location(0) vec4<f32> {
    return vec4<f32>(in.color, 1.0);
}`;

const SHADER_PHONG = BINDINGS + VS_PASS + FN_PHONG + /* wgsl */`
@fragment fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    return vec4<f32>(shadingPhong(normalize(in.normal), in.worldPos), 1.0);
}`;

const SHADER_BLINNPHONG = BINDINGS + VS_PASS + FN_BLINNPHONG + /* wgsl */`
@fragment fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    return vec4<f32>(blinnPhong(normalize(in.normal), in.worldPos), 1.0);
}`;

const SHADER_TOON = BINDINGS + VS_PASS + /* wgsl */`
@fragment fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    let N      = normalize(in.normal);
    let L      = normalize(mtl.lightPosShiny.xyz - in.worldPos);
    let V      = normalize(mtl.viewPosToon.xyz   - in.worldPos);
    let diff   = max(dot(N, L), 0.0);
    let steps  = mtl.viewPosToon.w;
    let stepped = floor(diff * steps) / steps;
    var color  = (mtl.colorAmbient.w + stepped) * mtl.colorAmbient.xyz * mtl.lightColorRough.xyz;
    if (dot(N, V) < 0.25) { color = vec3<f32>(0.0); }
    return vec4<f32>(clamp(color, vec3<f32>(0.0), vec3<f32>(1.0)), 1.0);
}`;

const SHADER_PBR = BINDINGS + VS_PASS + /* wgsl */`
const PI: f32 = 3.14159265359;
fn distGGX(NdotH: f32, a2: f32) -> f32 {
    let d = NdotH * NdotH * (a2 - 1.0) + 1.0;
    return a2 / (PI * d * d);
}
fn geomSchlick(v: f32, k: f32) -> f32 { return v / (v * (1.0 - k) + k); }
fn fresnelSchlick(cosT: f32, F0: vec3<f32>) -> vec3<f32> {
    return F0 + (1.0 - F0) * pow(clamp(1.0 - cosT, 0.0, 1.0), 5.0);
}
@fragment fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    let N = normalize(in.normal);
    let V = normalize(mtl.viewPosToon.xyz  - in.worldPos);
    let L = normalize(mtl.lightPosShiny.xyz - in.worldPos);
    let H = normalize(V + L);
    let NdotL = max(dot(N, L), 0.0);
    let NdotV = max(dot(N, V), 0.0);
    let NdotH = max(dot(N, H), 0.0);
    let HdotV = max(dot(H, V), 0.0);
    let rough = mtl.lightColorRough.w;
    let a = rough * rough; let a2 = a * a;
    let k = ((rough + 1.0) * (rough + 1.0)) / 8.0;
    let F0 = mix(vec3<f32>(0.04), mtl.colorAmbient.xyz, mtl.metallic);
    let D = distGGX(NdotH, a2);
    let G = geomSchlick(NdotV, k) * geomSchlick(NdotL, k);
    let F = fresnelSchlick(HdotV, F0);
    let spec = (D * G * F) / max(4.0 * NdotV * NdotL, 0.001);
    let kD   = (1.0 - F) * (1.0 - mtl.metallic);
    let Lo   = (kD * mtl.colorAmbient.xyz / PI + spec) * mtl.lightColorRough.xyz * NdotL;
    return vec4<f32>(mtl.colorAmbient.w * mtl.colorAmbient.xyz + Lo, 1.0);
}`;

export const WGSL_SHADERS: Record<ShadingMode, string> = {
  "none":        SHADER_NONE,
  "flat":        SHADER_FLAT,
  "gouraud":     SHADER_GOURAUD,
  "phong":       SHADER_PHONG,
  "blinn-phong": SHADER_BLINNPHONG,
  "toon":        SHADER_TOON,
  "pbr":         SHADER_PBR,
};
