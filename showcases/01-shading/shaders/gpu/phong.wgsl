// phong.wgsl – Klassisches Phong Shading: Specular via Reflect-Vektor (WebGPU)
...
@fragment fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    let R = reflect(-L, N);                                  // Reflect-Vektor (Phong)
    let diff = max(dot(N, L), 0.0);
    let spec = pow(max(dot(R, V), 0.0), mtl.lightPosShiny.w);
    return vec4<f32>(mtl.colorAmbient.w * mtl.colorAmbient.xyz + diff * mtl.colorAmbient.xyz * mtl.lightColorRough.xyz + spec * mtl.lightColorRough.xyz, 1.0);
}
